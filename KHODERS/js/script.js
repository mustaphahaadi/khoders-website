...
// Initialize scroll animations for consistent page experience
...
// Add page entrance animation for consistent experience
addPageEntranceAnimation() {
    ...
    sections.forEach((section, index) => {
        section.classList.add('section-animate', `animate-delay-${Math.min(index + 1, 6)}`);
    });

    // Add animations to cards
    const cards = document.querySelectorAll('.card, .service-card, .project-card, .team-card, .event-card, .blog-card');
    cards.forEach((card, index) => {
        card.classList.add('card-animate', `animate-delay-${Math.min((index % 3) + 1, 6)}`);
    });

    // Add text animations
    const textElements = document.querySelectorAll('.hero-text, .section-header');
    textElements.forEach((element, index) => {
        element.classList.add('text-animate', `animate-delay-${Math.min(index + 1, 3)}`);
    });

    // Add image animations
    const images = document.querySelectorAll('.hero-image, .featured-image');
    images.forEach((image, index) => {
        image.classList.add('image-animate', `animate-delay-${Math.min(index + 2, 4)}`);
    });
}
}
