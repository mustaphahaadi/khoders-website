...
$hasIdColumn = $tableExists ? admin_table_has_column($db, 'events', 'id') : false;
...
if ($db && $tableExists) {
    $columns = admin_filter_columns(
        $db,
        'events',
        ['id', 'title', 'description', 'date', 'time', 'location', 'category', 'created_at', 'updated_at'],
        ['id', 'title', 'date', 'time']
    );

    if (empty($columns)) {
        $columns = admin_get_columns($db, 'events');
    }
    ...
    $orderField = admin_preferred_order_field($columns, ['date', 'created_at', 'id']);

    try {
        $stmt = $db->query("SELECT $selectClause FROM events ORDER BY `$orderField` DESC");
...
function format_event_value(string $column, $value): string
{
    switch ($column) {
        case 'description':
            return admin_excerpt($value, 100);
        case 'date':
            return admin_safe($value ? date('M d, Y', strtotime($value)) : '');
        case 'time':
            return admin_safe($value ? date('h:i A', strtotime($value)) : '');
        case 'created_at':
        case 'updated_at':
...
<head>
    ...
    <title>Events - KHODERS Admin</title>
    <style>
    ...
    <main class="main-content">
        ...
        <div class="content-area">
            ...
            <div class="table-container">
                ...
                <?php if (!$tableExists): ?>
                    <p style="padding: 1.5rem; color: #B91C1C;">The events table is missing. Run the latest database migrations from <code>database/schema_updates.sql</code>.</p>
                <?php else: ?>
                    <table>
                        <thead>
                            <tr>
                                        ...
                                        <?php foreach ($columns as $column): ?>
                                            <td>
                                                <?php if ($column === 'description'): ?>
                                                    <details>
                                                        <summary><?php echo format_event_value($column, $event[$column] ?? ''); ?></summary>
                                                        <div class="description-full"><?php echo admin_safe($event[$column] ?? ''); ?></div>
                                                    </details>
                                                <?php else: ?>
                                                    <?php echo format_event_value($column, $event[$column] ?? ''); ?>
                                                <?php endif; ?>
                                            </td>
                                        <?php endforeach; ?>
                                        <?php if ($hasIdColumn): ?>
                                            <td class="actions">
                                                <a href="?action=delete&id=<?php echo admin_safe($event['id']); ?>" class="btn btn-danger" onclick="return confirm('Delete this event?');">Delete</a>
                                            </td>
                                        <?php endif; ?>
                                    </tr>
                                <?php endforeach; ?>
